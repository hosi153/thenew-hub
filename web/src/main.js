...
async function writeWithFallback(collection, docId, data, onStatus){
...
async function deleteWithFallback(collection, docId, onStatus){
  if(!sdkTransportBlocked){
    try{
...
async function mergeWithFallback(collection, docId, changes, onStatus){
  if(!sdkTransportBlocked){
    try{
      if(onStatus) onStatus('저장 중');
...
async function firestoreRestList(collection){
  const documents = [];
  let pageToken = '';
  do{
    const {signal, clear} = withAbortTimeout(REST_ATTEMPT_MS);
...
async function readWithFallback(collection){
  if(!sdkTransportBlocked){
    try{
...
async function createPasswordFields(pw){
...
async function authenticateItem(item, pw){
...
function toast(msg, ms){
  ...
  window._toastTimer = setTimeout(()=>t.classList.remove('show'), ms || 1800);
}
