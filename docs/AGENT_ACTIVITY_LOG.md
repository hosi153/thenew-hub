...
## 2026-08-31 (계속) — Claude (Sonnet 5, claude.ai 모바일 세션) — 인스타 공유: 가로스크롤 수정 + 정렬 방향 정정

**사용자 지시**: "가로 스크롤 안생기게 하고 최신 데이터가 위로오게해줘 내가 준 링크는 위에부터 과거 데이터야"

**원인 1 (가로스크롤)**: 인스타 아이디처럼 언더스코어(`_`)로 길게 이어진 문자열(예: `evergreen_sol___a`, `maniac_wedding11.17`)은 `word-break:break-word`만으로는 끊어지지 않음 — break-word는 "단어" 경계(공백/하이픈 등)에서만 줄바꿈하는데, 언더스코어는 그 경계로 인식 안 됨.

**수정 1**: 인스타/닉네임 컬럼에 `word-break:break-all`(강제로 어디서든 줄바꿈) + `overflow-wrap:anywhere` 추가.

**원인 2 (정렬 방향)**: 시드 데이터 53건에 전부 동일 시각을 넣어서, 실제로는 안정정렬 특성상 원문 입력 순서(프로도가 맨 위)로 표시되고 있었음. 사용자가 준 원문 리스트는 "위에서부터 과거" 순서였으므로, 반대로 표시되고 있었던 것.

**수정 2**: `SEED_INSTA`의 `createdAt`을 파이썬 스크립트로 재계산 — 원문 순서를 그대로 유지하면서, **첫 번째 항목(프로도)에 가장 이른 시각, 마지막 항목(2610 애참)에 가장 늦은 시각**을 1분 간격으로 부여. 최신순(createdAt 내림차순) 정렬 시 자연스럽게 원문 맨 아래 사람이 최상단에 오도록 정정.

**검증**:
- `npm run build` 성공
- 실제 정렬 결과를 파이썬으로 시뮬레이션해 상위 5명이 원문 하단 그룹(애참/뽀또섭/뽀또본/밍귤/낭맘), 하위 5명이 원문 상단 그룹(만덕/쩡이/헤이즐/몰랑/프로도)임을 확인
- `tests/vite-build.test.mjs`에 회귀 테스트 2개 추가 (긴 언더스코어 아이디 줄바꿈 처리 여부, 시드 53건의 createdAt이 순서대로 단조 증가하는지)
- **전체 45개 테스트 통과**
